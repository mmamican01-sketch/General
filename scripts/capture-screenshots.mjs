#!/usr/bin/env node
/**
 * Capture responsive screenshots at 375, 768, 1440 for EN pages.
 * Run: npm run build && node scripts/capture-screenshots.mjs
 * Requires: npx serve dist (or astro preview) - script will start serve automatically.
 */
...
const OUT_DIR = join(ROOT, "parity-screenshots", "responsive");
...
async function main() {
  ...
  // Ensure Chrome is available: npx puppeteer browsers install chrome
  ...
  for (const width of WIDTHS) {
    const outPath = join(OUT_DIR, String(width));
  ...
  await browser.close();
  serve.kill();
  console.log("\nScreenshots saved to parity-screenshots/responsive/{width}/");
}
